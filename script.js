(() => {
  ...
  const emailFallback=document.createElement('a');emailFallback.className='button button-outline';emailFallback.textContent='Email these details instead';emailFallback.hidden=true;formActions?.appendChild(emailFallback);
  const setFormStatus=(message='',type='')=>{if(!formStatus)return;formStatus.textContent=message;formStatus.hidden=!message;formStatus.className='form-status'+(type?' form-status-'+type:'');}; const resetSubmitButton=()=>{if(!submitButton)return;submitButton.disabled=false;submitButton.removeAttribute('aria-busy');submitButton.textContent=defaultSubmitLabel;};
